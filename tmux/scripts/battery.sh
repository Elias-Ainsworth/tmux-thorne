...
main()
{
  bat_label=$(get_tmux_option "@thorne-battery-label" "♥")
  if [ "$bat_label" == false ]; then
    bat_label=""
  fi

  no_bat_label=$(get_tmux_option "@thorne-no-battery-label" "AC")
  if [ "$no_bat_label" == false ]; then
    no_bat_label=""
  fi

  show_bat_label=$(get_tmux_option "@thorne-show-battery-status" false)
